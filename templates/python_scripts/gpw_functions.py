...
def map_financial_data(df, db_conn):
    ...
    try:
        ...
        df["WK"] = df["Year"].apply(
            lambda x: get_data(x, df=market_indicators, row="Wartość księgowa na akcję")
        )
        ...
        df["Zobowiązania długoterminowe"] = df["Year"].apply(
            lambda x: get_data(
                x[:4],
                df=BS,
        ...
        df["Zobowiązania krótkoterminowe"] = df["Year"].apply(
            lambda x: get_data(
                x[:4],
                df=BS,
                col_with_rownames="Inde",
        ...
        df["Przychody ze sprzedaży na akcję"] = df["Year"].apply(
            lambda x: get_data(
                x, df=market_indicators, row="Przychody ze sprzedaży na akcję"
            )
        )
        df["Zysk na akcję"] = df["Year"].apply(
            lambda x: get_data(x, df=market_indicators, row="Zysk na akcję")
        )
        df["Zysk operacyjny na akcję"] = df["Year"].apply(
            lambda x: get_data(x, df=market_indicators, row="Zysk operacyjny na akcję")
        )
        df["Enterprise Value na akcję"] = df["Year"].apply(
            lambda x: get_data(x, df=market_indicators, row="Enterprise Value na akcję")
        )
        df["EV / P"] = df["Year"].apply(
            lambda x: get_data(
                x, df=market_indicators, row="EV / Przychody ze sprzedaży"
            )
        )
        ...
        df["Cena / WK Grahama"] = df["Close"] / (
            df["Zobowiązania długoterminowe"] + df["Zobowiązania krótkoterminowe"]
        )
