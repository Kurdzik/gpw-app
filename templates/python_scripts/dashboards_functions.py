...
def get_and_plot_data(ticker, data_type="plot"):
    ...
    try:
        ...
        df_bs["Total liabilities"] = (
            df_bs["Aktywa razem"]
            - df_bs["Kapitał własny akcjonariuszy jednostki dominującej"]
            - df_bs["Kapitał (fundusz) podstawowy"]
            - df_bs["Udziały (akcje) własne"]
            - df_bs["Kapitał (fundusz) zapasowy"]
            - df_bs["Udziały niekontrolujące"]
        )
    except Exception:
        df_bs = None
    ...
    try:
        fig1 = px.bar(
            data_frame=df_rzis.rename(
                {"Przychody ze sprzedaży": "Sales Revenue", "Zysk netto": "Net Profit"},
                axis=1,
            ),
            y="Sales Revenue",
            x="index",
    ...
    try:
        fig3 = px.bar(
            data_frame=df_cf.rename(
                {
                    "Przepływy pieniężne z działalności operacyjnej": "CF from operations",
                    "Przepływy pieniężne razem": "Total CF",
                },
                axis=1,
            ),
