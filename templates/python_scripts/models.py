...
def select_ARMA(data):
    ...
    # best ARMA
    ARMA_params = results.loc[results["AIC_ARMA"] == results["AIC_ARMA"].min()][
        "order_ARMA"
    ].values[0]
...
def select_ARIMA(data):
    ...
    # best ARIMA
    ARIMA_params = results.loc[results["AIC_ARIMA"] == results["AIC_ARIMA"].min()][
        "order_ARIMA"
    ].values[0]
...
def train_and_register_model(model_name, dataset, ticker, tracking_uri):
    ...
    if "Holt" in model_name:
        try:
            model = ExponentialSmoothing(
                dataset, trend="mul", seasonal="mul", seasonal_periods=12
            ).fit()
    ...
    with mlflow.start_run(experiment_id=experiment.experiment_id, run_name=model_name):
        ...
        mlflow.sklearn.log_model(
            model, "model", registered_model_name=f"{model_name}_{ticker}"
        )
...
def load_model_and_plot(ticker, model_name, plot_last_mnths, conn, data_type="plot"):
    ...
    # Dataset parameters
    plot_data_since = pd.date_range(
        end=df.index.max(), periods=plot_last_mnths, freq="M"
    )[0].date()
    ...
    joint_data = pd.concat(
        [
            fitted_data.to_frame().rename(
                {0: "Predictions", "predicted_mean": "Predictions"}, axis=1
            ),
    ...
    # plot true data
    fig_1.add_scatter(
        x=joint_data.index, y=joint_data["True Value"], mode="lines", name="True Values"
    )

    # plot predictions
    fig_1.add_scatter(
        x=joint_data.index,
        y=joint_data["Predictions"],
        mode="lines",
        name="Predictions",
    )
    ...
    mape = mean_absolute_percentage_error(
        joint_data["True Value"], joint_data["Predictions"]
    )
    ...
    metrics_html = (
        metrics.to_html(index=False, justify="justify-all", col_space=250)
        + "<br>"
        + "<hr>"
    )
...
def predict_and_plot(ticker, model_name, fcst_period, plot_last_mnths, conn, data_type):
    ...
    forceast_offset = (
        len(pd.date_range(start=last_day_known_to_model, end=now, freq="M")) + 1
    )

    fcst_period += forceast_offset

    # Get index for forcated values - only Business Days, no weekends
    fcsted_index = pd.date_range(
        start=model.fittedvalues.index[-1], freq="B", periods=fcst_period
    )

    # Get forcasted values
    fcsted_vals = (
        model.forecast(fcst_period)
        .to_frame()
        .set_index(fcsted_index)
        .rename({0: "Close", "predicted_mean": "Close"}, axis=1)
    )
    ...
    if data_type == "html":
        full_html = (
            PREDICTIONS_FIRST_PART + fig.to_html()[55:-15] + PREDICTIONS_LAST_PART
        )
