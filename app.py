...
from templates.python_scripts.constants import (MODELS_FIRST_PART,
                                                MODELS_LAST_PART,
                                                PREDICTIONS_FIRST_PART,
                                                PREDICTIONS_LAST_PART)
from templates.python_scripts.dashboards_functions import get_and_plot_data
from templates.python_scripts.db_functions import (get_data_from_db,
                                                   test_db_conn)
from templates.python_scripts.models import (load_model_and_plot,
                                             predict_and_plot)
...
def run_data_loader():
    ...
    df = get_data_from_db(date_from=date_from, date_to=date_to, ticker=ticker)
    ...
    return render_template(
        "index_scrapper.html", download_ready=download_ready, file_name=file_name
    )
...
def init_conn():
    ...
    return (
        render_template(
            "index_db_conn.html", message=message, add_record_button=add_record_button
        ),
        approval,
        conn_string,
    )
...
def test_conn():
    ...
    message, message_link, conn_string, approval = test_db_conn(
        db_type=db_type,
        db_ip=db_ip,
        db_port=db_port,
        database=database,
...
def add_conn():
    ...
    try:
        pd.DataFrame([conn_string, db_table, db_schema, "active"]).T.rename(
            {0: "connection_string", 1: "table", 2: "schema", 3: "status"}, axis=1
        ).to_sql(
            "connections",
            if_exists="append",
            con=conn,
            schema="connected_dbs",
            index=False,
        )
    except Exception:
        message = "Something went wrong, connection was not added"
...
def confirm_removal():
    ...
    if len(df.loc[df.connection_string == conn_string]) == 0:
        ...
        remove_record_button = (
            "Click here to remove your Database credentials from registry"
        )

    return render_template(
        "index_db_conn_remove.html",
        message=message,
        remove_record_button=remove_record_button,
    )
...
def fundam_analysis():
    ...
    with open(
        f"templates/rendered_dashboards/dashboards_temp_{ticker}.html", "w"
    ) as file:
        file.write(html_div)
...
def run_model():
    ...
    try:
        ...
        html_div = (
            MODELS_FIRST_PART
            + "unexpected error occured, please try another combination of date and company ticker"
            + MODELS_LAST_PART
        )

    with open(
        f"templates/rendered_predictions/predictions_temp_{ticker}.html", "w"
    ) as file:
        file.write(html_div)
...
def predict():
    ...
    with open(
        f"templates/rendered_predictions/predictions_future_temp_{ticker}.html", "w"
    ) as file:
        file.write(html_div)

    return render_template(
        f"rendered_predictions/predictions_future_temp_{ticker}.html"
    )
